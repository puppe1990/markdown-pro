...
describe('imageService', () => {
  ...
  describe('resolveMarkdownImages', () => {
    it('should resolve image references to data URLs', () => {
      ...
      expect(resolved).toBe('![Alt text](data:image/png;base64,mockbase64image1)');
    });

    it('should resolve multiple image references', () => {
      const markdown = '![Image 1](image-ref://test-image-1) and ![Image 2](image-ref://test-image-2)';
      const resolved = resolveMarkdownImages(markdown);
      expect(resolved).toContain('data:image/png;base64,mockbase64image1');
      expect(resolved).toContain('data:image/png;base64,mockbase64image2');
    });
    ...
    it('should handle markdown with text before and after images', () => {
      const markdown = 'Before image ![Alt text](image-ref://test-image-1) After image';
      const resolved = resolveMarkdownImages(markdown);
      expect(resolved).toBe('Before image ![Alt text](data:image/png;base64,mockbase64image1) After image');
    });
    ...
    it('should handle images with special characters in alt text', () => {
      const markdown = '![Image with (parentheses)](image-ref://test-image-1)';
      const resolved = resolveMarkdownImages(markdown);
      expect(resolved).toBe('![Image with (parentheses)](data:image/png;base64,mockbase64image1)');
    });
  });
});
